...
const MoodScreen = () => {
  const { user } = useAuth();
  const { addMoodEntry, moodHistory } = usePomodoro();
  ...
  // Converter moodHistory do contexto para o formato MoodEntry da tela
  const recentEntries: MoodEntry[] = moodHistory.slice(0, 10).map((entry) => {
    // Mapear MoodLevel para o objeto Mood
    const moodLevelToMood: { [key: string]: Mood } = {
      VERY_GOOD: moods[0],
      GOOD: moods[1],
      NEUTRAL: moods[2],
      BAD: moods[3],
    ...
    return {
      ...
      mood: moodLevelToMood[entry.mood] || moods[2],
      note: entry.notes,
      energy: entry.energy || 3,
      stress: entry.stress || 2,
    };
  });

  // Calcular distribuição real de humores
  const calculateMoodDistribution = () => {
    if (moodHistory.length === 0) {
      return { great: 0, good: 0, neutral: 0, sad: 0, bad: 0 };
    }
    ...
    return {
      great: Math.round((counts.VERY_GOOD / total) * 100),
      good: Math.round((counts.GOOD / total) * 100),
      neutral: Math.round((counts.NEUTRAL / total) * 100),
      sad: Math.round((counts.BAD / total) * 100),
      bad: Math.round((counts.VERY_BAD / total) * 100),
    };
  };
  ...
  const handleSave = async () => {
    console.log("MoodScreen: handleSave chamado", {
      hasUser: !!user,
    ...
    try {
      setSaving(true);
      console.log("MoodScreen: Salvando humor no Firestore...");

      // Mapear emoji para MoodLevel
      const moodLevelMap: { [key: string]: MoodLevel } = {
        "😄": MoodLevel.VERY_GOOD,
        "�": MoodLevel.GOOD,
        "😐": MoodLevel.NEUTRAL,
        "😔": MoodLevel.BAD,
      ...
      console.log("MoodScreen: Dados do humor:", {
        emoji: selectedMood.emoji,
        moodLevel: moodLevelMap[selectedMood.emoji],
        energy,
        stress,
        hasNote: !!note,
      });
      ...
      const moodData: any = {
        userId: user.id,
        mood: moodLevelMap[selectedMood.emoji] || MoodLevel.NEUTRAL,
        energy,
        stress,
        timestamp: new Date(),
      };

      // Apenas adicionar notes se tiver conteúdo
      if (note && note.trim()) {
        moodData.notes = note.trim();
      }

      const moodId = await firestoreService.saveMoodEntry(moodData);

      // Criar objeto completo e adicionar ao contexto
      const newMoodEntry = {
        id: moodId,
        ...moodData,
      };
      ...
      console.log("MoodScreen: Humor salvo com sucesso!", moodId);

      // Resetar estados imediatamente para UI responsiva
      setSaving(false);
        ...
        >
          ...
          >
            ...
            <Text
              style={[
                styles.moodLabel,
                isSelected && { color: mood.color, fontWeight: "800" },
              ]}
            >
              {mood.label}
            </Text>
  ...
  const MoodEntryCard = ({
    ...
    return (
      ...
      >
        <LinearGradient colors={entry.mood.gradient} style={styles.entryCard}>
          <View style={styles.entryHeader}>
            ...
            <View style={styles.entryStats}>
              <View style={styles.entryStat}>
                ...
                <Text
                  style={[styles.entryStatText, { color: entry.mood.color }]}
                >
                  {entry.energy}
                </Text>
              </View>
                ...
                {recentEntries.length > 0 ? (
                  ...
                  <View style={styles.emptyState}>
                    ...
                    <Text style={styles.emptyText}>
                      Registre seu primeiro humor acima para começar!
                    </Text>
                  </View>
                )}
              </Animated.View>
              ...
              >
                ...
                >
                  ...
                  <Text style={styles.insightsText}>
                    Seu humor está 15% melhor comparado à semana passada!
